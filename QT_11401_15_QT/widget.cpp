...
{
    QStringList deleteTitles;
    bool hasPriorityTask = false;

    // 1️⃣ 掃描整棵 Tree，找出「被勾選的任務」
    for (int i = 0; i < ui->treeWidget->topLevelItemCount(); ++i) {
        ...
        for (int j = 0; j < root->childCount(); ++j) {
            ...
            if (item->checkState(0) == Qt::Checked) {
                deleteTitles << item->text(0);

                if (root == priorityRoot) {
    ...
    if (deleteTitles.isEmpty()) {
        QMessageBox::information(this, "提示", "請先勾選要刪除的任務");
        return;
    }
    ...
    // 3️⃣ 一次性刪除所有被勾選的任務（⭐重點）
    tasks.erase(
        std::remove_if(tasks.begin(), tasks.end(),
                       [&](const Task &t) {
                           if (t.date != currentDate) return false;

                           for (const QString &title : deleteTitles) {
                               if (title.contains(t.title)) {
                                   return true;
                               }
                           }
                           return false;
